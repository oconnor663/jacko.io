...
And finally the main polling loop can read from it:[^instant_only]

[^instant_only]: Since our `Waker` is a no-op, you might wonder why we need
    bother with `wake`. We've already finished sleeping, and we're about to
    poll again at the top of the main loop, so what's the point? Well, it turns
    out that fancy combinators like [`JoinAll`] (not our simple version, but
    the real one from the [`futures`] crate) construct a unique `Waker`
    internally for each of their children, and [they only poll children who've
    called `wake`][never_polled_again]. In short, the contract of
    `Future::poll` is that whenever we return `Pending`, we _must_ arrange to
    call `wake` later, one way or another.
